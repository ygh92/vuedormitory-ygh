<template>
    <el-container class="dormitory-info">
      ...
      <el-main class="main-content">
        <el-card>
          <span class="residents-info">居住人数: {{ currentResidents.length }}</span>
          寝室长: {{ currentResidents.length > 0 ? currentResidents[0].dorAdmin : '暂无' }}

          <el-collapse v-if="currentResidents.length > 0">
            <el-collapse-item v-for="resident in currentResidents" :key="resident.name" :title="resident.name">
              <el-row>
          ...
          <el-tag v-for="historicalResident in historicalResidents" :key="historicalResident.studentId" class="historical-tag">
            {{ historicalResident.name }} ({{ historicalResident.studentId }})
          </el-tag>
        </el-card>
      </el-main>
