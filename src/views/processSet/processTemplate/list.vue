<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="updateTime" width="165px" label="更新时间"/>
...
<!--          <el-button type="text" v-if="scope.row.status == 0" size="mini" @click="removeDataById(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.remove')  === false">删除</el-button></template>-->
          <el-button  type="text" size="mini" @click="publish(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.publish')  === false">发布</el-button>
          ...
          <el-button type="text"   size="mini" @click="removeDataById(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.remove')  === false">删除</el-button></template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    <el-pagination
